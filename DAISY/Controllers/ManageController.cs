    ...
    public class ManageController : Controller
    {
        ...
        public async Task<ActionResult> Index(ManageMessageId? message)
        {
            ViewBag.StatusMessage =
                ...
                : message == ManageMessageId.SetTwoFactorSuccess ? "Nhà cung cấp xác thực hai yếu tố của bạn đã được thiết lập."
                ...
                : message == ManageMessageId.AddImageSuccess ? "Ảnh đại diện của bạn đã được thêm."
        ...
        public async Task<ActionResult> VerifyPhoneNumber(VerifyPhoneNumberViewModel model)
        {
            ...
            ModelState.AddModelError("", "Không xác minh được số điện thoại");
            return View(model);
        }
        ...
        public async Task<ActionResult> SetPassword(SetPasswordViewModel model)
        {
            if (ModelState.IsValid)
            {
                ...
                if (result.Succeeded)
                {
                    ...
                    SendMail.SendEmail(user.Email, "Xác nhận đặt mật khẩu của bạn", "Chỉ còn một bước nữa để tạo mật khẩu cho tài khoản của bạn. Bạn vui lòng click vào <a href=\"" + callbackUrl + "\">đường dẫn này</a> để hoàn tất.", "");
        ...
        {
            ViewBag.StatusMessage =
                message == ManageMessageId.RemoveLoginSuccess ? "Thông tin đăng nhập bên ngoài đã bị xóa."
                : message == ManageMessageId.Error ? "Một lỗi đã xảy ra."
                : "";
            var user = await UserManager.FindByIdAsync(User.Identity.GetUserId());
            if (user == null)
